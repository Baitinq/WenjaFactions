...
public abstract class ArgumentExecutor implements CommandExecutor, TabCompleter {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (args.length < 1) {
			...
			sender.sendMessage(ChatColor.YELLOW + ChatColor.BOLD.toString() + WordUtils.capitalizeFully(this.label + ChatColor.GOLD + " - " + ChatColor.GRAY + "(Page 1/1)"));
			for (CommandArgument argument : this.arguments) {
				String permission = argument.getPermission();
				if (permission == null || sender.hasPermission(permission)) {
					sender.sendMessage(ChatColor.YELLOW + argument.getUsage(label) + ChatColor.GOLD + " - " + ChatColor.GRAY + argument.getDescription() + '.');
				}
			}
			sender.sendMessage(ChatColor.GOLD + BukkitUtils.STRAIGHT_LINE_DEFAULT);
			return true;
