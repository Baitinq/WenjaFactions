...
import me.joeleoli.hcfactions.listener.BorderListener;
...
public class EotwHandler {
	...
	public EotwHandler(FactionsPlugin plugin) {
		this.plugin = plugin;
	}
			...
			for (World.Environment current : World.Environment.values()) {
				...
				if (elapsedSeconds % BORDER_DECREASE_TIME_SECONDS == 0) {
					...
					for (Player player : Bukkit.getServer().getOnlinePlayers()) {
						if (player.getWorld().getEnvironment().equals(current)) player.sendMessage(msg);
					}
					...
					String msg2 = (ChatColor.DARK_AQUA + "Border decreasing to " + ChatColor.YELLOW + newBorderSize + ChatColor.DARK_AQUA + " blocks in " + ChatColor.YELLOW + BORDER_DECREASE_TIME_ALERT_WORDS + ChatColor.DARK_AQUA + '.');
					for (Player player : Bukkit.getServer().getOnlinePlayers()) {
						if (player.getWorld().getEnvironment().equals(current)) player.sendMessage(msg2);
					}
				}
			}
		}
