...
public class KothScheduleArgument extends CommandArgument {
	...
	public KothScheduleArgument(FactionsPlugin plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		for (Map.Entry<LocalDateTime, String> entry : scheduleMap.entrySet()) {
			...
			if (scheduleDateTime.isAfter(now)) { // only show the events that haven't been scheduled yet.
				...
				shownEvents.add("  " + colour + WordUtils.capitalizeFully(entry.getValue()) + ": " + ChatColor.YELLOW + weekName + ' ' + scheduleDateTime.getDayOfMonth() + ' ' + monthName + ChatColor.RED + " (" + HHMMA.format(scheduleDateTime) + ')' + ChatColor.GRAY + " - " + ChatColor.GOLD + DurationFormatUtils.formatDuration(now.until(scheduleDateTime, ChronoUnit.MILLIS), TIME_UNTIL_PATTERN));
			}
		}
