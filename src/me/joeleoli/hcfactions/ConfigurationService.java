...
public class ConfigurationService {
	...
	public static String DONATE_URL = "store.wenjapvp.net";
	public static int SPAWN_BUFFER = 200;
	public static int MAP_NUMBER = 8;
	...
	public static String SCOREBOARD_TITLE = ChatColor.GOLD.toString() + ChatColor.BOLD + "   Wenja " + ChatColor.RED + "[Map " + MAP_NUMBER + "]   ";
	...
	public static void init(JavaPlugin plugin) {
		...
		if (enc != null) {
			...
			for (String key : enc.getKeys(false)) {
				...
				if (e != null) ENCHANTMENT_LIMITS.put(e, enc.getInt(key));
				else plugin.getLogger().severe("Could not find enchantment with name " + key);
			}
		}

		for (Map.Entry<Enchantment, Integer> e : ENCHANTMENT_LIMITS.entrySet()) {
		...
		if (pot != null) {
			...
			for (String key : pot.getKeys(false)) {
				...
				if (potion != null) POTION_LIMITS.put(potion, pot.getInt(key));
				else plugin.getLogger().severe("Could not find potion with name " + key);
			}
		}

		for (Map.Entry<PotionType, Integer> e : POTION_LIMITS.entrySet()) {
		...
		if (road != null) {
			...
			for (String key : road.getKeys(false)) {
				...
				if (env != null) ROAD_LENGTHS.put(env, road.getInt(key));
				else plugin.getLogger().severe("Could not find world type with name " + key);
			}
		}
		...
		if (bor != null) {
			...
			for (String key : bor.getKeys(false)) {
				World.Environment env = World.Environment.valueOf(key);
				if (env != null) BORDER_SIZES.put(env, bor.getInt(key));
				else plugin.getLogger().severe("Could not find world type with name " + key);
			}
		}
		...
		if (spa != null) {
			...
			for (String key : spa.getKeys(false)) {
				World.Environment env = World.Environment.valueOf(key);
				if (env != null) SPAWN_RADIUS_MAP.put(env, spa.getInt(key));
				else plugin.getLogger().severe("Could not find world type with name " + key);
			}
		}
