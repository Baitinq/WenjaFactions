...
public class ChatListener implements Listener {
	...
	private String getFormattedMessage(Player player, PlayerFaction playerFaction, String message, CommandSender viewer) {
		...
		if (playersFaction != null) {
			return ChatColor.translateAlternateColorCodes('&', "&6[" + tag + "&6] &f" + player.getDisplayName() + "&f: ") + ChatColor.RESET + message;
		} else {
			return ChatColor.translateAlternateColorCodes('&', player.getDisplayName() + "&f: " + ChatColor.RESET) + message;
		}
	}
