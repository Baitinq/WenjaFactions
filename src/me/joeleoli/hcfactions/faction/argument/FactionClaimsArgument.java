...
public class FactionClaimsArgument extends CommandArgument {
	...
	public FactionClaimsArgument(FactionsPlugin plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length < 2) {
			...
			if (!(faction instanceof ClaimableFaction)) {
				sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "You can only check the claims of factions that can have claims.");
				return true;
			}
		...
		if (claims.isEmpty()) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "Faction " + targetFaction.getDisplayName(sender) + ChatColor.RED + " has no claimed land.");
			return true;
		}
		...
		if (sender instanceof Player && !sender.isOp() && (targetFaction instanceof PlayerFaction && ((PlayerFaction) targetFaction).getHome() == null)) {
			if (selfFaction != targetFaction) {
				sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "You cannot view the claims of " + targetFaction.getDisplayName(sender) + ChatColor.RED + " because their home is unset.");
				return true;
			}
		}

		sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.GOLD + "Claims of " + targetFaction.getDisplayName(sender) + ChatColor.GOLD + " (" + claims.size() + "):");

		for (Claim claim : claims) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.GRAY + " " + claim.getFormattedName());
		}
