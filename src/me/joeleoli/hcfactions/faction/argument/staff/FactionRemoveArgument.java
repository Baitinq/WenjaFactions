...
public class FactionRemoveArgument extends CommandArgument {
	...
	public FactionRemoveArgument(final FactionsPlugin plugin) {
	...
	private static class RemoveAllPrompt extends StringPrompt {
		...
		public RemoveAllPrompt(FactionsPlugin plugin) {
			this.plugin = plugin;
		}
		...
		public Prompt acceptInput(ConversationContext context, String string) {
			switch (string.toLowerCase()) {
				case "yes": {
					...
					Bukkit.broadcastMessage(ChatColor.RED.toString() + ChatColor.BOLD + "All factions have been disbanded" + (conversable instanceof CommandSender ? " by " + ((CommandSender) conversable).getName() : "") + '.');
