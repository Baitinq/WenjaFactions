...
import me.joeleoli.hcfactions.faction.event.FactionRemoveEvent;
...
public class ConquestTracker implements EventTracker, Listener {
	...
	public ConquestTracker(FactionsPlugin plugin) {
		this.plugin = plugin;
	...
	/**
	 * Gets the amount of points a {@link PlayerFaction} has gained for this {@link ConquestTracker}.
	 *
			...
			if (remainingMillis <= 0L) {
				...
				if (playerFaction != null) {
					...
					if (newPoints < ConfigurationService.CONQUEST_REQUIRED_WIN_POINTS) {
						...
						Bukkit.broadcastMessage(ChatColor.YELLOW + "[" + eventFaction.getName() + "] " + ChatColor.RED + ChatColor.BOLD + playerFaction.getName() + ChatColor.GOLD + " gained " + 1 + " point for capturing " + captureZone.getDisplayName() + ChatColor.GOLD + ". " + ChatColor.AQUA + '(' + newPoints + '/' + ConfigurationService.CONQUEST_REQUIRED_WIN_POINTS + ')');
					} else {
	...
	public void onControlLoss(Player player, CaptureZone captureZone, EventFaction eventFaction) {
		...
		if (remainingMillis > 0L && captureZone.getDefaultCaptureMillis() - remainingMillis > MINIMUM_CONTROL_TIME_ANNOUNCE) {
			Bukkit.broadcastMessage(ChatColor.YELLOW + "[" + eventFaction.getName() + "] " + ChatColor.RED + ChatColor.BOLD + player.getName() + ChatColor.GOLD + " was knocked off " + captureZone.getDisplayName() + ChatColor.GOLD + '.');
		}
	}
	...
	public void onPlayerDeath(PlayerDeathEvent event) {
		...
		if (currentEventFac instanceof ConquestFaction) {
			...
			if (playerFaction != null && ConfigurationService.CONQUEST_POINT_LOSS_PER_DEATH > 0) {
				...
				Bukkit.broadcastMessage(ChatColor.YELLOW + "[" + currentEventFac.getName() + "] " + ChatColor.GOLD + playerFaction.getName() + " lost " + ConfigurationService.CONQUEST_POINT_LOSS_PER_DEATH + " points because " + player.getName() + " died." + ChatColor.AQUA + " (" + newPoints + '/' + ConfigurationService.CONQUEST_REQUIRED_WIN_POINTS + ')' + ChatColor.YELLOW + '.');
			}
		}
	}
}
