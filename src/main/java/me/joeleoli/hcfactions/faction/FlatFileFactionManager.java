package me.joeleoli.hcfactions.faction;
...
public class FlatFileFactionManager implements Listener, FactionManager {
	...
	private Table<String, Long, Claim> claimPositionMap = HashBasedTable.create();
	private ConcurrentMap<UUID, UUID> factionPlayerUuidMap = new ConcurrentHashMap<>();
	private ConcurrentMap<UUID, Faction> factionUUIDMap = new ConcurrentHashMap<>();
	private Map<String, UUID> factionNameMap = new CaseInsensitiveMap<>();
	private HashMap<String, UUID> factionMutes = new HashMap<>();
	private FactionsPlugin plugin;
	private Config config;

	public FlatFileFactionManager(FactionsPlugin plugin) {
	...
	public void removeMute(UUID factionUUID) {
		factionMutes.remove(factionUUID);
	}

	public HashMap<String, UUID> getMutes() {
		return factionMutes;
	}

	public void addMute(String reason, UUID factionUUID) {
		factionMutes.put(reason, factionUUID);
	}

	public boolean isMuted(UUID factionUUID) {

		return factionMutes.containsValue(factionUUID);
	}
