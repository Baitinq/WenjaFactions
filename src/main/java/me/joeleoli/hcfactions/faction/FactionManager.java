...
public interface FactionManager {
	...
	HashMap<String, UUID> getMutes();

	boolean isMuted(UUID factionUUID);

	void addMute(String reason, UUID factionUUID);

	void removeMute(UUID factionUUID);
	...
	/**
	 * Gets a {@link PlayerFaction} containing a member who has a {@link UUID} String equal to the
	 * given search string.
	 *
	...
	/**
	 * Gets a {@link PlayerFaction} of a {@link Player}.
	 *
	 * @param player the {@link Player} to lookup
	 * @return the {@link PlayerFaction} containing {@link Player}
	 */
	...
	/**
	 * Gets a {@link Faction} with a given name or containing a {@link FactionMember} with the given
	 * name.
	 *
