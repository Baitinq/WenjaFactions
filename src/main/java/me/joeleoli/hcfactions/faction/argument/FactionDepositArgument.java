...
public class FactionDepositArgument extends CommandArgument {
	...
	public FactionDepositArgument(FactionsPlugin plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args[1].equalsIgnoreCase("all")) {
			...
			if ((amount = (JavaUtils.tryParseInt(args[1]))) == null) {
				sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "'" + args[1] + "' is not a valid number.");
				return true;
			}
		}
		...
		if (playerBalance < amount) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "You need at least " + EconomyManager.ECONOMY_SYMBOL + JavaUtils.format(amount) + " to do this, you only have " + EconomyManager.ECONOMY_SYMBOL + JavaUtils.format(playerBalance) + '.');
		...
		playerFaction.broadcast(FactionsPlugin.PREFIX + ConfigurationService.TEAMMATE_COLOUR + sender.getName() + ChatColor.YELLOW + " has deposited " + ChatColor.GREEN + EconomyManager.ECONOMY_SYMBOL + JavaUtils.format(amount) + ChatColor.YELLOW + " into the faction balance.");
