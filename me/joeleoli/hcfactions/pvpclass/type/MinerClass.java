package me.joeleoli.hcfactions.pvpclass.type;
...
import me.joeleoli.hcfactions.pvpclass.event.PvpClassEquipEvent;
...
public class MinerClass extends PvpClass implements Listener {
	...
	public MinerClass(FactionsPlugin plugin) {
	...
	private void removeInvisibilitySafely(Player player) {
		for (PotionEffect active : player.getActivePotionEffects()) {
			if (active.getType().equals(PotionEffectType.INVISIBILITY) && active.getDuration() > DEFAULT_MAX_DURATION) {
				player.sendMessage(ChatColor.GOLD + getName() + ChatColor.YELLOW + " invisibility has been" + ChatColor.GRAY + " disabled" + ChatColor.YELLOW + ".");
	...
	public void onClassEquip(PvpClassEquipEvent event) {
		...
		if (event.getPvpClass() == this && player.getLocation().getBlockY() <= INVISIBILITY_HEIGHT_LEVEL) {
			...
			player.sendMessage(ChatColor.GOLD + getName() + ChatColor.YELLOW + " invisibility has been" + ChatColor.GRAY + " enabled" + ChatColor.YELLOW + ".");
		}
	}

	/**
	 * Applies the {@link MinerClass} invisibility {@link PotionEffect} depending on the {@link
	 * Player}s {@link Location}.
	 *
