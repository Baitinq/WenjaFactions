package me.joeleoli.hcfactions.pvpclass.archer;
...
public class ArcherClass extends PvpClass implements Listener {
	...
	public ArcherClass(FactionsPlugin plugin) {
		...
		if (((entity instanceof Player)) && ((damager instanceof Arrow))) {
			...
			if ((source instanceof Player)) {
				...
				if ((this.plugin.getTimerManager().archerTimer.getRemaining((Player) entity) == 0L) || (this.plugin.getTimerManager().archerTimer.getRemaining((Player) entity) < TimeUnit.SECONDS.toMillis(5L))) {
					...
					shooter.sendMessage(com.doctordark.util.Color.translate("&e[&9Arrow Range &e(&c" + String.format("%.1f", Double.valueOf(distance)) + "&e)] " + "&6Marked " + damaged.getName() + " &6for 6 seconds. &9&l(1 heart)"));

					damaged.sendMessage(ChatColor.GOLD + "You were archer tagged by " + ChatColor.RED + shooter.getName() + ChatColor.GOLD + " from " + ChatColor.RED + String.format("%.1f", Double.valueOf(distance)) + ChatColor.GOLD + " blocks away.");
