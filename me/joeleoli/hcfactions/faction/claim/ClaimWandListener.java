package me.joeleoli.hcfactions.faction.claim;

import com.google.common.base.Predicate;
import me.joeleoli.hcfactions.FactionsPlugin;
import me.joeleoli.hcfactions.faction.type.PlayerFaction;
import me.joeleoli.hcfactions.visualise.VisualBlock;
...
public class ClaimWandListener implements Listener {
	...
	public ClaimWandListener(FactionsPlugin plugin) {
		this.plugin = plugin;
	}
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (action == Action.LEFT_CLICK_BLOCK || action == Action.RIGHT_CLICK_BLOCK) {
			...
			if (plugin.getClaimHandler().canClaimHere(player, blockLocation)) {
				...
				player.sendMessage(ChatColor.GREEN + "Set the location of claim selection " + ChatColor.GOLD + selectionId + ChatColor.GREEN + " to: " + ChatColor.GOLD + '(' + ChatColor.WHITE + blockX + ", " + blockZ + ChatColor.GOLD + ')');
				...
				if (claimSelection.hasBothPositionsSet()) {
					...
					player.sendMessage(ChatColor.GOLD + "Claim selection cost: " + (selectionPrice > playerFaction.getBalance() ? ChatColor.RED : ChatColor.GREEN) + EconomyManager.ECONOMY_SYMBOL + selectionPrice + ChatColor.GOLD + ". Current size: (" + ChatColor.WHITE + claim.getWidth() + ", " + claim.getLength() + ChatColor.GOLD + "), " + ChatColor.WHITE + claim.getArea() + ChatColor.GOLD + " blocks.");
				}
