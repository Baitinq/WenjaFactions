...
public class FactionLeaderArgument extends CommandArgument {
	...
	public FactionLeaderArgument(FactionsPlugin plugin) {
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		if (!(sender instanceof Player)) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "Only players can set faction leaders.");
			return true;
		}
		...
		if (selfRole != Role.LEADER) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "You must be the current faction leader to transfer the faction.");
			return true;
		}
		...
		if (targetMember == null) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "Player '" + args[1] + "' is not in your faction.");
			return true;
		}

		if (targetMember.getUniqueId().equals(uuid)) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "You are already the faction leader.");
			return true;
		}

		targetMember.setRole(Role.LEADER);
		selfMember.setRole(Role.CAPTAIN);
		playerFaction.broadcast(FactionsPlugin.PREFIX + ConfigurationService.TEAMMATE_COLOUR + selfMember.getRole().getAstrix() + selfMember.getName() + ChatColor.YELLOW + " has transferred the faction to " + ConfigurationService.TEAMMATE_COLOUR + targetMember.getRole().getAstrix() + targetMember.getName() + ChatColor.YELLOW + '.');
