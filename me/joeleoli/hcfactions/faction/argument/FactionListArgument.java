...
public class FactionListArgument extends CommandArgument {
	...
	public FactionListArgument(FactionsPlugin plugin) {
	...
	private void showList(final int pageNumber, final String label, final CommandSender sender) {
		...
		for (Map.Entry<PlayerFaction, Integer> entry : sortedMap) {
			...
			ComponentBuilder builder = new ComponentBuilder("  " + index + ". ").color(net.md_5.bungee.api.ChatColor.GOLD);
		...
		if (pageNumber > maxPages) {
			sender.sendMessage(FactionsPlugin.PREFIX + ChatColor.RED + "There " + (maxPages == 1 ? "is only " + maxPages + " page" : "are only " + maxPages + " pages") + ".");
			return;
		}

		sender.sendMessage(ChatColor.GOLD + BukkitUtils.STRAIGHT_LINE_DEFAULT);
		sender.sendMessage(ChatColor.YELLOW + ChatColor.BOLD.toString() + " FACTION LIST " + ChatColor.GOLD + "-" + ChatColor.GRAY + " (Page " + pageNumber + '/' + maxPages + ')');
