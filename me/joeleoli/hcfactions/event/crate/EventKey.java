...
public class EventKey extends Key {
	...
	public ItemStack getItemStack(EventKeyData eventKeyData) {
		...
		ItemMeta meta = stack.getItemMeta();
		meta.setDisplayName(getColour() + getName() + " Key");
		meta.setLore(Lists.newArrayList(ChatColor.GRAY + "Right click an empty Chest to use.", ChatColor.GRAY + eventKeyData.getEventType().getDisplayName() + ChatColor.YELLOW + " Inventory " + ChatColor.GOLD + eventKeyData.inventoryNumber));
		stack.setItemMeta(meta);
		return stack;
	}
